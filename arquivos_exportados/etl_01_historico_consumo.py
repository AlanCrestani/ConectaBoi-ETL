#!/usr/bin/env python3
"""
ETL Script para 01_historico_consumo
...
def load_config():
    ...
    with open('config/01_historico_consumo_config.json', 'r', encoding='utf-8') as f:
        return json.load(f)
...
def load_to_staging(df):
    ...
    try:
        ...
        # Inserir no Supabase
        response = supabase.table('etl_staging_01_historico_consumo').insert(records).execute()
...
def main():
    ...
    try:
        logger.info("Iniciando ETL para 01_historico_consumo")
        
        # Carregar configuração
        config = load_config()
        
        # Carregar CSV
        df = pd.read_csv('C:/conectaboi_csv/01_historico_consumo.csv')
        ...
        if success:
            ...
            import shutil
            shutil.move(f'C:/conectaboi_csv/01_historico_consumo.csv', f'C:/conectaboi_csv/processed/01_historico_consumo_{datetime.now().strftime("%Y%m%d_%H%M%S")}.csv')
