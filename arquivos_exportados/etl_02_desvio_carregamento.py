#!/usr/bin/env python3
"""
ETL Script para 02_desvio_carregamento
...
def load_config():
    ...
    with open('config/02_desvio_carregamento_config.json', 'r', encoding='utf-8') as f:
        return json.load(f)
...
def load_to_staging(df):
    ...
    try:
        ...
        # Inserir no Supabase
        response = supabase.table('etl_staging_02_desvio_carregamento').insert(records).execute()
...
def main():
    ...
    try:
        logger.info("Iniciando ETL para 02_desvio_carregamento")
        
        # Carregar configuração
        config = load_config()
        
        # Carregar CSV
        df = pd.read_csv('C:/conectaboi_csv/02_desvio_carregamento.csv')
        ...
        if success:
            ...
            import shutil
            shutil.move(f'C:/conectaboi_csv/02_desvio_carregamento.csv', f'C:/conectaboi_csv/processed/02_desvio_carregamento_{datetime.now().strftime("%Y%m%d_%H%M%S")}.csv')
