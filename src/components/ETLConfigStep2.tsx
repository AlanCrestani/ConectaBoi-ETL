...
const ETLConfigStep2 = ({ fileId, csvHeaders, sqlSchema, csvData, onNext, onBack }: ETLConfigStep2Props) => {
  ...
  useEffect(() => {
    ...
    const extractColumns = (schema: string) => {
      ...
      lines.forEach(line => {
        ...
        if (trimmed && !trimmed.startsWith('CREATE') && !trimmed.startsWith('(') && !trimmed.startsWith(')') && !trimmed.startsWith('--')) {
          const columnMatch = trimmed.match(/^(\w+)/);
          if (columnMatch && !['id', 'batch_id', 'uploaded_at', 'processed'].includes(columnMatch[1])) {
  ...
  const getTypeColor = (type: string) => {
    switch(type) {
      case 'direct': return 'bg-primary text-primary-foreground';
      case 'derived': return 'bg-accent text-accent-foreground';
      case 'fixed': return 'bg-secondary text-secondary-foreground';
                ...
                <div>
                  ...
                  {mapping.type === 'fixed' || mapping.type === 'derived' ? (
                    <Input
                      ...
                      onChange={(e) => updateMapping(index, 'sqlColumn', e.target.value)}
                      placeholder="nome_da_coluna"
                    />
                  ) : (
                    <Select
                      value={mapping.sqlColumn}
                      onValueChange={(value) => updateMapping(index, 'sqlColumn', value)}
                    >
                      <SelectTrigger>
                ...
                <div>
                  <Label>
                    {mapping.type === 'fixed' ? 'Valor Fixo' : 
                     mapping.type === 'derived' ? 'Derivado de' : 'Coluna CSV'}
                  </Label>
                  {mapping.type === 'fixed' ? (
                    <Input
                    ...
                    <Select
                      value={mapping.type === 'derived' ? mapping.derivedFrom : mapping.csvColumn}
                      onValueChange={(value) => 
                        updateMapping(index, mapping.type === 'derived' ? 'derivedFrom' : 'csvColumn', value)
                      }
                    >
                      <SelectTrigger>
              ...
              {mapping.sqlColumn === 'id_curral' && (
                <div className="flex items-center space-x-2">
                  <input
                    ...
                    onChange={(e) => updateMapping(index, 'validateInDimCurral', e.target.checked)}
                    className="rounded"
                  />
