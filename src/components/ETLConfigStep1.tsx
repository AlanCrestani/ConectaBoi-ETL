...
const ETLConfigStep1 = ({ fileId, onNext }: ETLConfigStep1Props) => {
  ...
  processed BOOLEAN DEFAULT FALSE
    ...
    const schema = schemaTemplates[tableName] || `CREATE TABLE ${tableName} (
  id BIGSERIAL PRIMARY KEY,
  data DATE NOT NULL,
  batch_id UUID DEFAULT gen_random_uuid(),
  uploaded_at TIMESTAMP DEFAULT NOW(),
    ...
    reader.onload = (e) => {
      ...
      if (skipFirstLine && parsedData.length > 1) {
        ...
        toast({
          ...
          description: "A primeira linha foi removida e a segunda linha agora é o cabeçalho.",
        });
      }

      const csvHeaders = processedData[0];
      const csvRows = processedData.slice(1).filter(row => row.join('').length > 30); // Filter short rows
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center space-x-2">
              ...
              {csvUploaded && <Check className="h-5 w-5 text-success ml-auto" />}
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            <div className="flex items-center space-x-2 p-3 bg-muted/50 rounded-lg">
              <Checkbox 
                ...
                onCheckedChange={(checked) => setSkipFirstLine(checked as boolean)}
              />
              <label htmlFor="skip-first-line" className="text-sm text-muted-foreground cursor-pointer">
            ...
            {csvUploaded && (
              <div className="bg-success/10 border border-success/20 rounded-lg p-4">
                <p className="text-sm text-success-foreground">
                  ✓ Arquivo carregado: {csvData.length} linhas, {headers.length} colunas
                </p>
                <div className="mt-2">
                  <p className="text-xs text-muted-foreground mb-1">Colunas encontradas:</p>
                  <div className="flex flex-wrap gap-1">
                    ...
                    {headers.length > 5 && (
                      <span className="text-xs text-muted-foreground">+{headers.length - 5} mais...</span>
                    )}
                  </div>
                </div>
              </div>
